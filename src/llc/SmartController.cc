...
#if DEBUG
#define dprintf(...) logARQ(LOGINFO, __VA_ARGS__)
#else /* !dprintf */
#define dprintf(...)
#endif /* !DEBUG */
...
void SmartController::received(std::shared_ptr<RadioPacket> &&pkt)
{
    ...
    if (pkt->hdr.nexthop != kNodeBroadcast &&
        pkt->hdr.nexthop != radionet_->getMyNodeId())
        return;
    ...
    if (pkt->hdr.nexthop != radionet_->getMyNodeId())
        return;
...
void SmartController::ack(RecvWindow &recvw)
{
    ...
    pkt->hdr.curhop = radionet_->getMyNodeId();
    pkt->hdr.nexthop = recvw.node.id;
    pkt->hdr.flags = {0};
    pkt->hdr.seq = {0};
    pkt->ehdr().data_len = 0;
    pkt->ehdr().src = radionet_->getMyNodeId();
...
void SmartController::nak(RecvWindow &recvw, Seq seq)
{
    ...
    logARQ(LOGDEBUG, "send nak: node=%u; nak=%u",
...
void SmartController::broadcastHello(void)
{
    ...
    if (time_master && *time_master == radionet_->getMyNodeId())
        radionet_->foreach([&] (Node &node) {
            std::lock_guard<std::mutex> lock(node.timestamps_mutex);
            auto                        last_timestamp = node.timestamps.rbegin();

            if (node.id != radionet_->getMyNodeId() && last_timestamp != node.timestamps.rend()) {
                logTimeSync(LOGDEBUG, "Echoing timestamp: node=%u; t_sent=%f; t_recv=%f",
                    (unsigned) node.id,
...
void SmartController::retransmit(SendWindow::Entry &entry)
{
    ...
    logARQ(LOGDEBUG, "retransmit: node=%u; seq=%u; mcsidx=%u",
...
void SmartController::drop(SendWindow::Entry &entry)
{
    ...
    logARQ(LOGDEBUG, "dropping packet: node=%u; seq=%u",
        (unsigned) sendw.node.id,
        (unsigned) entry.pkt->hdr.seq);
    ...
    for(auto it = pkt.begin(); it != pkt.end(); ++it) {
        switch (it->type) {
            ...
            case ControlMsg::Type::kTimestamp:
            {
                ...
                logTimeSync(LOGDEBUG, "Timestamp: node=%u; t_sent=%f; t_recv=%f",
                    (unsigned) pkt.hdr.curhop,
...
void SmartController::handleCtrlTimestampEchos(RadioPacket &pkt, Node &node)
{
    ...
    if (node.id != radionet_->getMyNodeId() && time_master && node.id == *time_master) {
        for(auto it = pkt.begin(); it != pkt.end(); ++it) {
            switch (it->type) {
                case ControlMsg::Type::kTimestampEcho:
                {
                    if (it->timestamp_echo.node == radionet_->getMyNodeId()) {
                        MonoClock::time_point t_sent;
                        MonoClock::time_point t_recv;
                        ...
                        logTimeSync(LOGDEBUG, "Timestamp echo: node=%u; t_sent=%f; t_recv=%f",
                            (unsigned) pkt.hdr.curhop,
                            (double) t_sent.get_real_secs(),
                            (double) t_recv.get_real_secs());
                    }
...
inline void appendSelectiveACK(NetPacket &pkt,
                               ...
                               Seq end)
{
    logARQ(LOGDEBUG, "send selective ack: node=%u; seq=[%u, %u)",
        (unsigned) recvw.node.id,
        (unsigned) begin,
        (unsigned) end);
        ...
        switch (it->type) {
            case ControlMsg::Type::kNak:
            {
                ...
                // If this packet has already been ACK'ed, ignore the NAK.
                    ...
                    // Log the NAK
                    logARQ(LOGDEBUG, "nak: node=%u; seq=%u",
                        (unsigned) sendw.node.id,
                        (unsigned) it->nak);
    ...
    for(auto it = pkt.begin(); it != pkt.end(); ++it) {
        switch (it->type) {
            case ControlMsg::Type::kSelectiveAck:
            {
                ...
                if (!sawACKRun) {
                    logARQ(LOGDEBUG, "selective ack: node=%u; per_end=%u",
                        (unsigned) node.id,
                ...
                if (nextSeq < it->ack.begin) {
                    logARQ(LOGDEBUG, "selective nak: node=%u; seq=[%u,%u)",
                        (unsigned) node.id,
                        (unsigned) nextSeq,
                ...
                logARQ(LOGDEBUG, "selective ack: node=%u; seq=[%u,%u)",
                    (unsigned) node.id,
                    (unsigned) it->ack.begin,
                    (unsigned) it->ack.end);
